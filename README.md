...
Запуск по расписанию без присмотра (ночной прогон канала, отчёты, уведомления): [docs/AUTONOMOUS.md](docs/AUTONOMOUS.md)
...
5. **Analyze (Staged)**: *LLM находит → Python доказывает → детерминированный отбор выбирает → LLM пишет метаданные.* Обзор эпизода → scout по чанкам с перекрытием (только интервал и дословная цитата) → цитата ищется в транскрипте, неподтверждённые кандидаты отбрасываются → cleanup и judge отвечают решениями keep/drop/merge по `candidate_id` и не могут сдвинуть клип или переписать цитату; judge видит реальные первые и последние секунды клипа. Дальше — снап границ без потери цитаты, аудио-замеры и MMR-отбор с квотами типов, политикой пересечений и разнообразием тем. Артефакты — в `output/<file_stem>/<модель>/` (например, `gemma4_26b/`).
...
output/
  my_podcast/
    ...
    .forge_state.json          # Отпечатки входов стадий: что пересчитать при изменениях
    gemma4_26b/                # Папка модели анализа
      ...
      cleaned_candidates.json  # После проверки цитат, дедупа, cleanup и аудио-замеров
      rejected_candidates.json # Всё, что отбросили фильтры, с причиной
      analysis_metrics.json    # Метрики прогона: выживаемость, цитаты, дубли, время стадий
      analysis_complete.json   # Анализ дошёл до конца (пустой результат — тоже результат)
      llm_cache/               # Кэш ответов LLM для перезапуска после сбоя
      ...
      reels/                   # Нарезанные .mp4 клипы
        ...
        rejected.json          # Отбракованные моменты с причинами
        rejected/              # Клипы, не прошедшие фильтры или проверку (если их кодировали)
      reels_preview.mp4        # Превью всех клипов одним файлом
  _runs/
    latest.json                # Отчёт последнего прогона (см. docs/AUTONOMOUS.md)
```
