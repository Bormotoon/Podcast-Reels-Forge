...
1. **Распознавание речи (faster-whisper)**: Преобразование аудио/видео в текст с пословными таймкодами. Модель `large-v3`, два режима — быстрый батчевый и точный с контекстом (см. [Режимы запуска под задачи](#режимы-запуска-под-задачи)).
