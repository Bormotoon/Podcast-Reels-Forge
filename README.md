...
Этапы: `transcribe`, `diarize`, `proofread`, `article`, `analyze`, `cut`. Опечатка в названии — ошибка, а не молчаливый пропуск половины конвейера. Пропущенный этап не мешает остальным: если вычитанный транскрипт уже есть от прошлого запуска, `--only article` возьмёт именно его.
